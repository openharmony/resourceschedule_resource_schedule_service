    ...
    const std::vector<sptr<WindowVisibilityInfo>>& windowVisibilityInfo)
{
    ...
    for (auto& info : windowVisibilityInfo) {
        ...
        ResSchedUtils::GetInstance().ReportDataInProcess(ResType::RES_TYPE_WINDOW_VISIBILITY_CHANGE, isVisible ?
            ResType::WindowVisibilityStatus::VISIBILITY : ResType::WindowVisibilityStatus::NOT_VISIBILITY, payload);
    }
}
} // namespace ResourceSchedule
} // namespace OHOS
