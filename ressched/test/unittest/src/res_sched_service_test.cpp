...
HWTEST_F(ResSchedServiceTest, RemoteRequest001, Function | MediumTest | Level0)
{
    ...
    MessageParcel reply;
    int32_t res = resSchedServiceStub_->OnRemoteRequest(ResourceScheduleInterfaceCode::REPORT_DATA, reply, reply, option);
    EXPECT_TRUE(res);
    res = resSchedServiceStub_->OnRemoteRequest(ResourceScheduleInterfaceCode::KILL_PROCESS, reply, reply, option);
    EXPECT_TRUE(res);
