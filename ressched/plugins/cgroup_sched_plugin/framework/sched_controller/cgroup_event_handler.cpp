...
void CgroupEventHandler::HandleReportAudioCapTureState(uint32_t resType, int64_t value, const nlohmann::json& payload)
{
    int32_t uid = 0;
    int32_t pid = 0;
    if (!supervisor_) {
...
void CgroupEventHandler::HandleNapModeEvent(uint32_t resType, int64_t value, const nlohmann::json& payload)
{
    ...
    if (uid <= 0 || pid <= 0) {
        CGS_LOGE("%{public}s : uid or pid is invalid", __func__);
        return;
    }
    std::shared_ptr<Application> app = supervisor_->GetAppRecord(uid);
    std::shared_ptr<ProcessRecord> procRecord = app ? app->GetProcessRecord(pid) : nullptr;
    if (!app || !procRecord) {
        CGS_LOGE("%{public}s : app or procRecord is nullptr", __func__);
        return;
    }
