...
cgroup_sched_plugin 是资源调度子系统的进程分组调度插件，负责根据系统事件感知进程状态变化，决策进程所属的 cgroup 调度组，并通过内核 cgroup 文件系统或 ressched_executor（SA 1918）将进程/线程迁移至对应的调度组，实现前台/后台/顶级应用等场景的差异化 CPU 调度。
