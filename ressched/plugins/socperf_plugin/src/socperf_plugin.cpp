    ...
    for (const Item& item : itemLists.itemList) {
        for (SubItem sub : item.subItemList) {
            if (sub.name == ENABLE_STRING) {
                ...
                Utils::AddKeyAppName(sub.value, keyAppName_);
            } else if (sub.name == KEY_APP_TYPE) {
                Utils::AddKeyAppType(sub.value, keyAppType_);
            }
        }
    }
}
...
void SocPerfPlugin::InitPolicyMode()
{
    ...
    if (!socperfPolicy.itemList.empty() && !limitPolicy.itemList.empty()) {
        std::string policyMode = Utils::GetPolicyMode(socperfPolicy);
        Utils::ApplyPolicyLimits(limitPolicy, policyMode, powerLimit_, thermalLimit_);
    }
...
void SocPerfPlugin::AddOtherEventToFunctionMap()
{
    ...
    functionMap.insert(std::make_pair(RES_TYPE_SOCPERF_PERFORMANCE_MODE_CHANGED,
        [this](const std::shared_ptr<ResData>& data) { HandlePerformanceModeChange(data); }));
...
void SocPerfPlugin::InitResTypes()
{
    resTypes = {
        ...
        RES_TYPE_OOBE_CLONE,
        RES_TYPE_SOCPERF_PERFORMANCE_MODE_CHANGED,
    };
    InitOtherResTypes();
}
...
void SocPerfPlugin::HandleScreenOn()
{
    ...
    if (performanceMode_ == PerformanceMode::PERFORMANCE_MODE) {
        OHOS::SOCPERF::SocPerfClient::GetInstance().PerfRequestEx(
            PERF_REQUEST_CMD_ID_SCREEN_OFF_PERFORMANCE_MODE, false, "");
        OHOS::SOCPERF::SocPerfClient::GetInstance().PerfRequestEx(
            PERF_REQUEST_CMD_ID_SCREEN_ON_PERFORMANCE_MODE, true, "");
    }
...
void SocPerfPlugin::HandleScreenOff()
{
    ...
    if (screenStatus_ == SCREEN_OFF) {
        ...
        if (performanceMode_ == PerformanceMode::PERFORMANCE_MODE) {
            OHOS::SOCPERF::SocPerfClient::GetInstance().PerfRequestEx(
                PERF_REQUEST_CMD_ID_SCREEN_ON_PERFORMANCE_MODE, false, "");
            OHOS::SOCPERF::SocPerfClient::GetInstance().PerfRequestEx(
                PERF_REQUEST_CMD_ID_SCREEN_OFF_PERFORMANCE_MODE, true, "");
        }
...
bool SocPerfPlugin::HandleSceenModeBoost(const std::string& deviceModeType)
{
    ...
    if (deviceMode_ == DISPLAY_MODE_FULL || deviceMode_ == DISPLAY_MODE_GLOBAL_FULL || deviceMode_ == DISPLAY_MODE_MAIN) {
...
bool SocPerfPlugin::HandlePerformanceModeChange(const std::shared_ptr<ResData>& data)
{
    if (data == nullptr) {
        return false;
    }

    SOC_PERF_LOGI("SocPerfPlugin: socperf->HandlePerformanceMode: %{public}lld", (long long)data->value);
    if (data->value < static_cast<int64_t>(PerformanceMode::POWER_MODE_MIN) ||
        data->value > static_cast<int64_t>(PerformanceMode::POWER_MODE_MAX)) {
        SOC_PERF_LOGE("SocPerfPlugin: value[%{public}lld] invalid", (long long)data->value);
        return false;
    }

    std::lock_guard<ffrt::mutex> xmlLock(screenMutex_);
    performanceMode_ = static_cast<PerformanceMode>(data->value);
    if (performanceMode_ == PerformanceMode::PERFORMANCE_MODE) {
        if (screenStatus_ == SCREEN_ON) {
            OHOS::SOCPERF::SocPerfClient::GetInstance().PerfRequestEx(
                PERF_REQUEST_CMD_ID_SCREEN_OFF_PERFORMANCE_MODE, false, "");
