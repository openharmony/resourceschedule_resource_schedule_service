...
/**
 * @tc.name: HandleEventSlide
 * @tc.desc: Test whether HandleEventSlide interface is normal
 * @tc.type: FUNC
 */
HWTEST_F(FrameAwarePluginTest, HandleEventSlideTest, TestSize.Level1)
{
    uint32_t type = 0;
    int64_t value = 0;
    nlohmann::json payload;
    ...
    std::shared_ptr<ResData> data = std::make_shared<ResData>(type, value, payload);
    std::shared_ptr<ResData> data1 = std::make_shared<ResData>(type, value, payloadarray);
    data->resType = RES_TYPE_SLIDE_RECOGNIZE;
    data->value = 0;
    data1->resType = RES_TYPE_SLIDE_RECOGNIZE;
    data1->value = 0;
