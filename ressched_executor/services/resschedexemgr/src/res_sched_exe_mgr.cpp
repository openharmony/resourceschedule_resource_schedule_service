...
namespace {
    ...
    const std::map<uint32_t, std::string> resTypeToStr = {
        ...
        { ResExeType::RES_TYPE_STANDBY_EXECUTOR_ASYNC_EVENT, "RES_TYPE_STANDBY_EXECUTOR_ASYNC_EVENT" },
...
void ResSchedExeMgr::InitPluginMgr(const nlohmann::json& payload)
{
    ...
    isInit = true;
    std::vector<std::string> configStrs = payload.value(STR_CONFIG_READER, std::vector<std::string>());
    std::vector<std::string> switchStrs = payload.value(STR_PLUGIN_SWITCH, std::vector<std::string>());
